...
orderdetails.getByProductIdAndSize = (product_id, size, callback) => {
  const sqlString = "SELECT * FROM orderdetails WHERE product_id = ? AND size = ?";
  db.query(sqlString, [product_id, size], (err, result) => {
    if (err) return callback(err);
    callback(result);
  });
};
