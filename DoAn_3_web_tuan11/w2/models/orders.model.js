...
orders.getByUserId = (id, callback) => {
  const sqlString = "SELECT * FROM orders WHERE user_id = ? ";
  db.query(sqlString, id, (err, result) => {
    if (err) return callback(err);
    callback(result);
  });
...
orders.getTotal = (callback) => {
  const sqlString = "SELECT COUNT(*) AS total FROM orders";
  db.query(sqlString, (err, result) => {
    if (err) return callback(err);
    callback(null, result[0].total);  // Trả về giá trị total từ result
  });
};

orders.getTotalAmount = (callback) => {
  const sqlString = "SELECT SUM(total_amount) AS total_amount FROM orders ";
  db.query(sqlString, (err, result) => {
    if (err) return callback(err);
    callback(result);
  });
