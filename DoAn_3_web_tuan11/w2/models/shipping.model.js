...
shipping.getByOrderId = (id, callback) => {
  const sqlString = "SELECT * FROM shipping WHERE order_id = ? ";
  db.query(sqlString, id, (err, result) => {
    if (err) return callback(err);
    callback(result);
  });
...
shipping.updateStatus = (shipping, id, callBack) => {
  const sqlString = "UPDATE shipping SET ? WHERE order_id = ?";
