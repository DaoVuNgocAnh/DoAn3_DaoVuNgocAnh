        ...
        try {
            ...
            if (type === 'payment') {
                ...
                if (payment?.payment_status !== 'completed') {
                    ...
                    toast.current.show({ severity: 'success', summary: 'Thanh toán', detail: 'Đã cập nhật trạng thái thanh toán.', life: 3000 });
                }
            }
            ...
            if (type === 'shipping') {
                ...
                if (shipping?.shipping_status !== 'delivered') {
                    ...
                    toast.current.show({ severity: 'success', summary: 'Giao hàng', detail: 'Đã cập nhật trạng thái giao hàng.', life: 3000 });
                }
            }

            // Kiểm tra nếu cả hai trạng thái đã hoàn tất
            ...
            if (isPaymentDone === 'completed' && isShippingDone === 'delivered') {
                ...
                for (const detail of detailsRes.data) {
                    const productRes = await axios.get(`${API_BASE}/Products/${detail.product_id}`);
                    ...
                    if (!product || product.stock < detail.quantity) {
                        toast.current.show({
                            severity: 'error',
                            summary: 'Thiếu hàng',
                            detail: `Xin lỗi, sản phẩm "${product?.product_name || 'Không rõ'}" không đủ hàng trong kho. Đơn hàng sẽ bị huỷ.`,
                            life: 4000
                        });

                        // ❌ Xoá đơn hàng nếu không đủ hàng trong kho
                    ...
                    await axios.put(`${API_BASE}/Products/${detail.product_id}`, {
                        ...product,
                        stock: newStock
                    });
                }

                toast.current.show({ severity: 'success', summary: 'Hoàn tất', detail: 'Đơn hàng đã hoàn tất và trừ kho.', life: 3000 });
                fetchOrders();
                setStatusDialog(false);
            }
        } catch (err) {
            console.error('Lỗi cập nhật trạng thái hoặc cập nhật kho:', err);
            toast.current.show({ severity: 'error', summary: 'Lỗi', detail: 'Không thể cập nhật trạng thái hoặc trừ kho.', life: 3000 });
        }
    };
        ...
        <div className="p-4">
            ...
            <Dialog visible={detailDialog} header="Chi tiết đơn hàng" style={{ width: '800px' }} modal onHide={() => setDetailDialog(false)}>
                ...
                <DataTable value={orderDetails} responsiveLayout="scroll">
                    ...
                    <Column field="price" header="Giá gốc" body={(row) => formatCurrency(row.price)} />
                    <Column field="totalPrice" header="Thành tiền" body={(row) => formatCurrency(row.totalPrice)} />
                </DataTable>
