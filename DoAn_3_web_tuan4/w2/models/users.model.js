...
users.getById = (id, callback) => {
  const sqlString = "SELECT user_id, fullname, username, email, phone_number, address, image_user FROM users WHERE user_id = ? ";
...
users.getByUsernamePass = (username, password, callback) => {
  const sqlString = "SELECT user_id, fullname, username, email, phone_number, address, image_user FROM users WHERE username = ? AND password = ?";
